...
export async function loadPhotosFromDir(subDir: string): Promise<PhotoData[]> {
  const photosDir = path.join(process.cwd(), "public", "photos", subDir);
  ...
  for (const file of files) {
    ...
    photos.push({ src: `/photos/${subDir}/${file}`, alt: path.parse(file).name, exif });
  }
  return photos;
}
